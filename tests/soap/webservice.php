...
class LeelabotWebservice
{
	...
	public function Webservice($id, $data)
	{
		...
		if($data['query'] == 'POST')
		{
			$this->_SoapServer->service($data['rawPOST']);
			print_r($data['rawPOST']);
			$this->_main->BufferSetRawData($id, $this->_SoapServer->response);
		}
		else
		{
	...
	public function loadWebservice()
	{
		...
		$this->_SoapServer->setDebugLevel(1);
		$this->_SoapServer->register('LeelabotWebservice.hello');
	}
	
	public function hello($person)
	{
		$this->_main->BufferAppendData($this->_id, 'Hello, '.$person.' !');

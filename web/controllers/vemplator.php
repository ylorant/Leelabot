...
class vemplator {
	...
	*/
	...
	function __construct() {
		$this->baseDirectory = './'; // default to document root
	...
	public function output($template) {
		...
		foreach($template as $t) {
			if(file_exists($this->baseDirectory.$t)) {
				$out .= $this->bufferedOutput($this->baseDirectory, $t);
				$foundTemplate = true;
				break; // found the template, so don't check any more directories
			}
		}
		if(!$foundTemplate)
			die('Template (' . $t . ') not found in ' . getcwd());
		return $out;
	}
	...
	private function bufferedOutput($path, $template) {
		...
		$out = ob_get_clean();
		unlink($this->compileDirectory . $template . '.php');
		return $out;
	}

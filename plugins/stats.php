...
class PluginStats extends Plugin
{
	...
	private function IrcAwards($_ircplugin)
	{
		$cmd = $_ircplugin->_cmd;
	...
	private function _printAwards()
	{
		...
		$_ircplugin->sendMessage("\002Awards :\002 ".join(' | ', $buffer));
	}
	
	// TODO Afficher Stats avec foreach sur $this->config['ShowStats']
	private function IrcStats($_ircplugin)
	{
		$cmd = $_ircplugin->_cmd;
		$server = $_ircplugin->_nameOfServer(2, FALSE);
		$actual = Server::getName();
		
		if(isset($cmd[1]) && $server !== FALSE) //Il faut un paramètre : le joueur
		{
			if($server !== false)
			{
				Server::setServer($this->_main->servers[$server]);
				...
				else
				{
					...
					$_ircplugin->sendMessage("\002Stats de ".$player->name."\002 : ".$hits."\037Kills\037 : ".$_stats[$player->id]['kills']." - \037Deaths\037 : ".$_stats[$player->id]['deaths']." - \037Ratio\037 : ".$ratio.$caps." - \037Streaks\037 : ".$_stats[$player->id]['streaks']);
